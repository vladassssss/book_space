    ...
    public function findReviewsWithUsersByBookId(int $bookId): array
{
  ...
  public function save(Review $review): bool
    ...
    public function addReview(int $bookId, string $userName, int $rating, string $comment): bool {
        try {
            ...
            $stmt->bindParam(':user_name', $bindUserName, PDO::PARAM_STR);
    ...
    public function updateReview(int $reviewId, string $userName, int $rating, string $comment): bool {
        try {
            $sql = "UPDATE reviews SET rating = :rating, review_text = :review_text, updated_at = NOW() WHERE id = :review_id AND user_name = :user_name";
            $stmt = $this->db->prepare($sql);

            $bindReviewId = $reviewId;
            $bindRating = $rating;
            $bindComment = $comment;
            $bindUserNameForUpdate = $userName;

            $stmt->bindParam(':rating', $bindRating, PDO::PARAM_INT);
            $stmt->bindParam(':review_text', $bindComment, PDO::PARAM_STR);
            $stmt->bindParam(':review_id', $bindReviewId, PDO::PARAM_INT);
            $stmt->bindParam(':user_name', $bindUserNameForUpdate, PDO::PARAM_STR);
