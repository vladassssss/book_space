<?php
session_start();require_once __DIR__ . '/../app/Database/Connection.php';
...
require_once __DIR__ . '/../app/Controllers/BookstoreController.php';require_once __DIR__ . '/../app/Repositories/ReviewRepository.php';
require_once __DIR__ . '/../app/Services/ReviewService.php';
require_once __DIR__ . '/../app/Controllers/ReviewController.php';

use App\Database\Connection;
use App\Controllers\ReviewController;
use App\Controllers\BookstoreController;$bookId = $_GET['id'] ?? null;
if (!$bookId) {
    die("Не вказано id книги.");
}$db = Connection::getInstance()->getConnection();$bookstoreRepository = new \App\Repositories\BookstoreRepository($db);
$bookstoreService = new \App\Services\BookstoreService($bookstoreRepository);
$bookstoreController = new BookstoreController($bookstoreService);$book = $bookstoreController->getBookById($bookId);
if (!$book) {
...
$reviewController = new ReviewController($reviewService);if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_review') {
    $rating = isset($_POST['rating']) ? (int)$_POST['rating'] : 0;
    $comment = $_POST['comment'] ?? '';    if (!isset($_SESSION['user_id'])) {
        header("Location: login.php");
        exit();
    }
    $userId = $_SESSION['user_id'];    $reviewController->addReview($bookId, $userId, $rating, $comment);    header("Location: book.php?id=" . $bookId);
    exit();
